#!/usr/bin/env bash
# Installs the MateClaw Chrome Native Messaging host manifest on Linux.
# Make executable if needed: chmod +x install-linux.sh
set -euo pipefail

BRIDGE_PATH="${1:?usage: install-linux.sh /absolute/path/to/bridge EXTENSION_ID}"
EXTENSION_ID="${2:?usage: install-linux.sh /absolute/path/to/bridge EXTENSION_ID}"
